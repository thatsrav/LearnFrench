...
- **Listening TTS (natural speech only)**: With `OPENAI_API_KEY` set, the API exposes `POST /api/tts/french` (body: `{ "text": "…" }`, optional `"voice"`: `nova` | `alloy` | …) and returns MP3 (`tts-1-hd`). Point **`VITE_API_BASE_URL`** (web) and **`EXPO_PUBLIC_API_BASE_URL`** (Expo, e.g. LAN IP + port) at that server. There is **no** browser or on-device speech fallback (robotic). Units without `audio_uri` can still be practiced via “read transcript” when the API isn’t configured. For exam-faithful audio, add real **`audio_uri`** assets in the listening catalog.
